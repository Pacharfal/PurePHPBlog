...
class Prispevky extends Administrace
{
    ...
    public function ulozPrispevek()
    {
        ...
        if (!empty($podminka)) {
            $ulozeno = $this->db->vloz('prispevky', $data, $podminka);
        } else {
        ...
        if ($ulozeno !== false) {
            ...
            header('Location: ' . $this->zaklad->db . 'prispevky.php?oznameni=' . urlencode($oznameni));
            exit();
        } else {
            $chyba = 'Nepodařilo se odstranit příspěvek. ' . 'Zkuste to prosím později';
            require_once 'sablony/editace-prispevku.php';
        }
    }
