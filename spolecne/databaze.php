...
class Databaze
{
    ...
    public $dbserver = '';
    public $uzivatelskeJmeno = 'admin';
    public $heslo = 'HwFka)Q7OmQpWG[C';
    public $databaze = '';
    public $db = '';
    ...
    /*
        public function vyber($tabulka, $sloupce, $podminka = NULL) {
            ...
            $dotaz = $this->db->prepare("SELECT $sloupceSQL FROM '$tabulka'". $podminkaSQL);
            try {
    ...
    */
    ...
    public function vloz($tabulka, $data, $podminka)
    {
        $sloupce = array();
        $hodnoty = array();
        $parametry = array();
        ...
        $dotaz = $this->db->prepare("INSERT INTO '$tabulka'($sloupceSQL) VALUES ($hodnotySQL)");
    ...
    public function aktualizuj($tabulka, $data, $podminka)
    {
        $podminkaSQL = '';
        $parametry = array();
        if (is_array($podminka) && !empty($data)) {
            foreach ($data as $sloupec => $hodnota) {
        ...
        $dotaz = $this->db->prepare("UPDATE '$tabulka' SET $sloupceHodnotySQL" . $podminkaSQL);
        try {
            $dotaz->execute($parametry);
